 ...
 * 
 * Limitations: - Only supports the compiled condition type codes (up to 4) - Only supports a
 * feature bitmask of 1 byte in length - Assumes fingerprint length of less than 16777215 bytes -
 * Only accepts a MaxFulfillmentLength of 16777215 or less
 * 
 * @author adrianhopebailie
 */
public class ConditionOerOutputStream extends OerOutputStream {

  public ConditionOerOutputStream(OutputStream stream) {
    super(stream);
  }

  /**
   * Write the condition to the underlying stream using OER encoding per the specification:
   * 
  ...
  protected void writeFeatures(EnumSet<FeatureSuite> features) throws IOException {
    ...
    stream.write(FeatureSuiteOerEncoding.getBitmaskFromFeatures(features));
