...
public class OerInputStream extends InputStream {
  ...
  public long read32BitUInt() throws IOException {
    // TODO: UnitTest write32BitUInt
    long byte4 = stream.read();
    verifyNotEOF((int) byte4);
    long byte3 = stream.read();
    verifyNotEOF((int) byte3);
    long byte2 = stream.read();
    verifyNotEOF((int) byte2);
    long byte1 = stream.read();
    verifyNotEOF((int) byte1);
  ...
  public byte[] readOctetString(int minSize, int maxSize)
    ...
    if (length < minSize)
      throw new UnsupportedLengthException("Length indicator indicates the OCTET STRING to be "
          + length + " bytes which is less than the minimum size of " + minSize + "bytes.");

    if (length > maxSize)
      throw new UnsupportedLengthException("Length indicator indicates the OCTET STRING to be "
          + length + " bytes which is greater than the maximum size of " + maxSize + "bytes.");
