...
public class TestRsaSha256Fulfillment {
  ...
  // String pkcs1_privkey = "-----BEGIN RSA PRIVATE KEY-----\n"
  ...
  // PrivateKeyInfo key_info = PrivateKeyInfo.getInstance(pkcs1_privkey.getBytes("UTF-8"));
  // PrivateKey key = BouncyCastleProvider.getPrivateKey(key_info);
  //
  ...
  // Note: With PSS padding the generated signature will be different each new run, not
  // deterministic. Next one is just a "random" one
  ...
  // Build from secrets.
  // RsaSha256Fulfillment ffFromSecrets =
  // RsaSha256FulfillmentImpl.fromPrivateKeyAndMessage((RSAPrivateKey) key, message);
